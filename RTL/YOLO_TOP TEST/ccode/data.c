...
#include "ff.h"
#include "string.h"
#define IN_FILE_NAME "matrix_ori.dat"
#define OUT_FILE_NAME "matrix_exp.dat"
FATFS fs;
...
int sd_card_data_rw(){
    ...
    */
    ...
    printf(SD_Receive_Data,SD_Transfer_Data)
    //-----------------------------------------------------
...
// Write Data Function
void sd_write_data(char wr_dat[], u64 wr_len){
	FIL fil;
	UINT bw;
	f_open(&fil,OUT_FILE_NAME,FA_CREATE_ALWAYS | FA_WRITE | FA_READ);
	f_lseek(&fil, 0);
	f_write(&fil,wr_dat,wr_len,&bw);
	f_close(&fil);
}
// Read Data Function
void sd_read_data(char rd_dat[], u64 rd_len){
	FIL fil;
	UINT br;
	f_open(&fil,IN_FILE_NAME,FA_READ);
	f_lseek(&fil, 0);
